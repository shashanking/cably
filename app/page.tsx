...
import MapComponent, { GeoLayer } from '../components/MapComponent'
...
export default function Home() {
  ...
  const [sideTab, setSideTab] = useState<'layers' | 'charts'>('layers')
    ...
    <div className="flex h-[calc(100vh-52px)]">
      ...
      <aside className="w-[320px] min-w-[320px] bg-white border-r border-slate-200 flex flex-col overflow-hidden">
        ...
        <div className="flex border-b border-slate-200 shrink-0">
          {(['layers', 'charts'] as const).map(tab => (
            <button key={tab} onClick={() => setSideTab(tab)} className={`flex-1 py-2.5 text-xs font-semibold transition-all border-b-2 -mb-px ${sideTab === tab ? 'text-blue-600 border-blue-600 bg-blue-50/50' : 'text-slate-400 border-transparent hover:text-slate-600'}`}>
              {tab === 'layers' ? '🗂 Layers' : '📊 Analytics'}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 relative overflow-hidden flex flex-col">
        <div className="flex-1 relative">
          ...
          <MapComponent layers={allLayers} selectedFeature={selectedFeature} onFeatureClick={(f, c, ln) => { const layer = allLayers.find(l => l.geojson.features.includes(f)); setFeatureInfo({ feature: f, color: c, layerId: layer?.id || '', layerName: ln || layer?.name || '' }); setSelectedFeature(f) }} />
